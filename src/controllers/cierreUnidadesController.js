...
exports.cerrarUnidad = async (req, res) => {
  ...
  try {
    ...
    // ========================================
    // CALCULAR NOTAS_UNIDAD PARA TODOS LOS ALUMNOS (OPTIMIZADO EN BULK)
    // ========================================
    console.log('📊 Calculando notas_unidad para todos los alumnos...');

    // Obtener todos los alumnos que tienen calificaciones en esta unidad (EN BULK)
    const [alumnosConNotas] = await sequelize.query(`
      SELECT DISTINCT
        c.IdAlumno,
        COALESCE(SUM(CASE WHEN a.TipoActividad = 'zona' THEN c.Punteo ELSE 0 END), 0) AS NotaZona,
        COALESCE(SUM(CASE WHEN a.TipoActividad = 'final' THEN c.Punteo ELSE 0 END), 0) AS NotaFinal,
        (COALESCE(SUM(CASE WHEN a.TipoActividad = 'zona' THEN c.Punteo ELSE 0 END), 0) +
         COALESCE(SUM(CASE WHEN a.TipoActividad = 'final' THEN c.Punteo ELSE 0 END), 0)) AS NotaTotal
    ...
    console.log(`📝 Total alumnos a procesar: ${alumnosConNotas.length}`);

    // Insertar/actualizar TODAS las notas en una sola operación bulk
    if (alumnosConNotas.length > 0) {
      const valoresSQL = alumnosConNotas.map(alumno => {
        const notaTotal = parseFloat(alumno.NotaTotal);
        const aprobado = notaTotal >= 60 ? 1 : 0;

        return `(
          ${idUnidad},
          ${alumno.IdAlumno},
          ${unidad.IdAsignacionDocente},
          ${parseFloat(alumno.NotaZona)},
          ${parseFloat(alumno.NotaFinal)},
          ${notaTotal},
          ${aprobado},
          NOW(),
          ${idUsuario || 'NULL'},
      ...
      await sequelize.query(`
        INSERT INTO notas_unidad (
          IdUnidad, IdAlumno, IdAsignacionDocente, NotaZona, NotaFinal, NotaTotal,
          Aprobado, FechaRegistro, RegistradoPor, Estado
        ) VALUES ${valoresSQL}
      ...
      console.log(`✅ ${alumnosConNotas.length} notas calculadas e insertadas exitosamente`);
    }
